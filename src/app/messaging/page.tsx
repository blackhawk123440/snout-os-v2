...
function MessagesContent() {
  ...
  const provider = integrationData?.data?.messagingProvider ?? 'none';
        ...
        {!hasProvider && (
          <Section>
            <div className="flex items-center gap-3 rounded-xl border border-border-default bg-surface-secondary px-4 py-3">
              <Settings className="h-5 w-5 text-text-tertiary shrink-0" />
              <div className="flex-1">
                <p className="text-sm font-medium text-text-primary">SMS messaging not configured</p>
                <p className="text-xs text-text-tertiary">
                  In-app messages work without a provider. Connect Twilio or OpenPhone in{' '}
                  <a href="/settings" className="font-medium text-accent-primary hover:underline">Settings</a>{' '}
                  to enable SMS.
                </p>
              </div>
            </div>
