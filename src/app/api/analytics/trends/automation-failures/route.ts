/**
 * GET /api/analytics/trends/automation-failures?range=7d|30d|90d
 * Daily count of automation.failed + automation.dead events. Real DB-backed.
 * Owner/admin only.
 */
...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    return NextResponse.json(
      { error: 'Failed to load automation failures trend', message },
      { status: 500 }
    );
  }
}
