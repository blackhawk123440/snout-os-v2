...
vi.mock("@/lib/payout/payout-queue", () => ({
  ...
  payoutQueue: {},
  initializePayoutWorker: vi.fn(),
}));

vi.mock("@/lib/calendar-queue", () => ({
  enqueueCalendarSync: vi.fn().mockResolvedValue(undefined),
}));
...
import { enqueuePayoutForBooking, getPayoutJobId } from "@/lib/payout/payout-queue";
...
describe("getPayoutJobId", () => {
  it("returns deterministic jobId per bookingId", () => {
